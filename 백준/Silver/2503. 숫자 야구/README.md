...
1. **완전 탐색(브루트 포스) 접근법**
    - 가능한 모든 세 자리 숫자(1-9 범위, 서로 다른 숫자)를 생성
    - 각 숫자에 대해 모든 힌트를 검증하여 가능한 정답 카운트
2. **단계별 검증**
    - 숫자 중복 여부 확인
    - 각 힌트에 대한 스트라이크/볼 계산
    - 계산 결과와 힌트 결과 일치 여부 확인
3. **효율적인 탈출 조건**
    - 하나의 힌트라도 만족하지 않으면 즉시 다음 숫자로 진행
    - 모든 힌트를 만족하는 경우만 정답 카운트 증가
